...
ts_label = now_ist.strftime('%Y-%m-%d %H:%M:%S GMT+5:30')
st.markdown(
    f"**Application Environment Temporal Anchor (IST):** `{ts_label}` "
...
def ingest_and_sanitize(file_io, name_tag=""):
    ...
    for c in df.columns:
        ...
        if norm_c in ['officeid', 'office_id']:
...
if st.sidebar.button("🚀 Process Operational Records", use_container_width=True):
    ...
    TRANSIT_COLS = [
        ...
        'D1 Delivered', 'D1 Redirected', 'D1 Returned'
    ]
    ...
    def compile_transit_family(dfs_list):
        ...
        for c in TRANSIT_COLS:
            if c in concatenated.columns:
                concatenated[c] = pd.to_numeric(concatenated[c], errors='coerce').fillna(0.0)
            else:
                concatenated[c] = 0.0
    ...
    if raw_06 is not None:
        ...
        for c in p_cols:
            if c in raw_06.columns:
                raw_06[c] = pd.to_numeric(raw_06[c], errors='coerce').fillna(0.0)
            else:
                raw_06[c] = 0.0
    ...
    d_d0_sum = core_ledger['doc_D0 Delivered'] + core_ledger['doc_D0 Redirected'] + core_ledger['doc_D0 Returned']
    d_d1_sum = core_ledger['doc_D1 Delivered'] + core_ledger['doc_D1 Redirected'] + core_ledger['doc_D1 Returned']
    a_d0_sum = core_ledger['all_D0 Delivered'] + core_ledger['all_D0 Redirected'] + core_ledger['all_D0 Returned']
    a_d1_sum = core_ledger['all_D1 Delivered'] + core_ledger['all_D1 Redirected'] + core_ledger['all_D1 Returned']

    core_ledger['val_par_d0'] = generate_ratio_vector(core_ledger, p_d0_sum, 'par_Received')
    core_ledger['val_par_d1'] = generate_ratio_vector(core_ledger, p_d1_sum, 'par_Received')
    core_ledger['val_doc_d0'] = generate_ratio_vector(core_ledger, d_d0_sum, 'doc_Received')
    core_ledger['val_doc_d1'] = generate_ratio_vector(core_ledger, d_d1_sum, 'doc_Received')
    core_ledger['val_all_d0'] = generate_ratio_vector(core_ledger, a_d0_sum, 'all_Received')
    core_ledger['val_all_d1'] = generate_ratio_vector(core_ledger, a_d1_sum, 'all_Received')
    ...
    style_sub_header  = workbook_obj.add_format({
        'bg_color': '#DCE6F1', 'font_color': '#1F497D', 'bold': True, 
    ...
    def populate_type_b_sheet(ws, dataset):
        ...
        for sdn in subdivs:
            ...
            t_line = block_references[sdn]
            ws.write_formula(cursor, 3, f"=D{t_line}", format_int)
            ws.write_formula(cursor, 4, f"=E{t_line}", format_pct)
            ws.write_formula(cursor, 5, f"=F{t_line}", format_pct)
            ws.write_formula(cursor, 6, f"=G{t_line}", format_int)
            ws.write_formula(cursor, 7, f"=H{t_line}", format_pct)
            ws.write_formula(cursor, 8, f"=I{t_line}", format_pct)
            ws.write_formula(cursor, 9, f"=J{t_line}", format_int)
            ws.write_formula(cursor, 10, f"=K{t_line}", format_pct)
            ws.write_formula(cursor, 11, f"=L{t_line}", format_pct)
            ws.write_formula(cursor, 12, f"=M{t_line}", format_pct)
            ws.write_formula(cursor, 13, f"=N{t_line}", format_pct)
            ws.write_formula(cursor, 14, f"=O{t_line}", format_pct)
    ...
    for item_idx, r in df_defaulters.iterrows():
        ...
        ws5.write_formula(def_idx, 7, f"=IFERROR(S{def_idx+1}/G{def_idx+1}, \"\")", format_pct)
        ws5.write(def_idx, 8, r['doc_Received'], format_int)
        ws5.write_formula(def_idx, 9, f"=IFERROR(T{def_idx+1}/I{def_idx+1}, \"\")", format_pct)
        ws5.write(def_idx, 10, r['all_Received'], format_int)
        ws5.write_formula(def_idx, 11, f"=IFERROR(U{def_idx+1}/K{def_idx+1}, \"\")", format_pct)
        ws5.write_formula(def_idx, 12, f"=IFERROR(V{def_idx+1}/K{def_idx+1}, \"\")", format_pct)
        ws5.write(def_idx, 13, r['val_all_not_invoiced'], format_int)
        
        ws5.write_formula(def_idx, 14, f"=IFERROR(W{def_idx+1}/X{def_idx+1}, \"\")", format_pct)
        ws5.write_formula(def_idx, 15, f"=IFERROR(Y{def_idx+1}/Z{def_idx+1}, \"\")", format_pct)
        ws5.write_formula(def_idx, 16, f"=IFERROR(AA{def_idx+1}/AB{def_idx+1}, \"\")", format_pct)
